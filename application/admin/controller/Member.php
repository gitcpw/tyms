...
class Member extends Admin {
    ...
    public function ajaxindex(){
        ...
        {
            $first_leader = DB::query("select first_leader,count(1) as count  from tyms_users where first_leader in(".  implode(',', $user_id_arr).")  group by first_leader");
            $first_leader = convert_arr_key($first_leader,'first_leader');
            
            $second_leader = DB::query("select second_leader,count(1) as count  from tyms_users where second_leader in(".  implode(',', $user_id_arr).")  group by second_leader");
            $second_leader = convert_arr_key($second_leader,'second_leader');            
            
            $third_leader = DB::query("select third_leader,count(1) as count  from tyms_users where third_leader in(".  implode(',', $user_id_arr).")  group by third_leader");
        ...
        if(IS_POST){
            ...
            if(accountLog($user_id,$user_money,$pay_points,$return_info['desc'],0,$return_info['order_id'],$return_info['order_sn'])){
                if($return_id>0){  //有退货id,是订单退款，要更新退货单状态
                    ...
                    if($res)
                        $this->success("操作成功", url("Admin/order/return_info", array('id' => $return_id)));
                    $this->error("操作失败");
                }
                $this->success("操作成功",url("Admin/User/account_log",array('id'=>$user_id)));
            }else{
                $this->error("操作失败");
            }
            exit;
