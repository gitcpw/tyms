...
class Admin extends Base {
	...
	protected function setMenu() {
		...
		$row = db('menu')->field('id,title,url,icon,"" as style')->where($where)->order("sort asc,id asc")->select();
		...
		if ($pid) {
			...
			$map['hide'] = 0;
			$map['type'] = 'admin';
			$row         = db('menu')->field('id,title,url,icon,group,pid,"" as style')->where($map)->order("sort asc,id asc")->select();
			foreach ($row as $key => $value) {
				if (IS_ROOT || $this->checkRule($value['url'], 2, null)) {
					if ($controller == $value['url']) {
