...
final class Data
{
     ...
     * @return array
     */
    static public function menu($data, $pid = 0, $html = "&nbsp;", $fieldPri = 'cid', $fieldPid = 'pid', $level = 1)
    {
        if (empty($data)) {
            return array();
        }
        ...
        foreach ($data as $k=>$v) {
            ...
            if ($v[$fieldPid] == $pid) {
                $arr[$v[$fieldPri]] = $v;
                $arr[$v[$fieldPri]]["children"] = self::menu($data, $v[$fieldPri], $html, $fieldPri, $fieldPid, $level + 1);
                if(!$arr[$v[$fieldPri]]["children"]) unset($arr[$v[$fieldPri]]["children"]);
            }
        }
